...
<div class="page-header flex justify-between items-end mb-5">
    <div class="header-content">
        <h1 class="text-3xl font-bold m-0"><i class="fas fa-file-signature text-primary mr-2"></i> Documents Queue</h1>
                ...
                <tbody>
                    <?php if (!empty($pendingDocuments)): ?>
                        <?php foreach ($pendingDocuments as $doc): ?>
                            <tr class="hover-bg-light transition <?= $doc['status'] === 'pending' ? 'bg-warning-light' : '' ?>" style="<?= $doc['status'] === 'pending' ? 'opacity: 0.95;' : 'opacity: 0.75;' ?>">
                                <td class="py-4 pl-5">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-muted text-xs flex items-center"><i class="fas fa-paperclip mr-1"></i> <?= htmlspecialchars(basename($doc['file_path'] ?? 'document')) ?></span>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <a href="/admin/players/<?= htmlspecialchars($doc['player_id']) ?>" class="text-dark font-bold hover-text-primary transition d-flex items-center">
                                        ...
                                        Player #<?= htmlspecialchars($doc['player_id']) ?>
                                    </a>
                                </td>
                                <td>
                                    <div class="text-dark font-medium"><?= htmlspecialchars(date('M d, Y', strtotime($doc['created_at'] ?? 'now'))) ?></div>
                                </td>
                                <td class="text-center">
                                    <span class="badge bg-white <?= $doc['status'] === 'verified' ? 'text-success border-success' : ($doc['status'] === 'rejected' ? 'text-danger border-danger' : 'text-warning border-warning') ?> border rounded-pill px-3 py-1 shadow-sm">
                                ...
                                <td class="text-right pr-5">
                                    ...
                                    <?php else: ?>
                                        <span class="text-muted text-sm font-medium"><i class="fas fa-check-circle text-success mr-1"></i> Reviewed</span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="5" class="text-center py-5 text-muted">
                                <i class="fas fa-inbox text-4xl mb-3 text-light"></i>
